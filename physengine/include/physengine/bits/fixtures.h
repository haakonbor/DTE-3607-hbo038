...
namespace dte3607::physengine::fixtures
{
  ...
  struct FixtureLevel1 {
    ...
    // RigidBody types
    using RigidBody    = rb::RigidBody;
    using RigidBodyPtr = std::unique_ptr<RigidBody>;
    using RigidBodies  = std::vector<RigidBodyPtr>;

    using RBShape    = rb::rb_shapes::Shape;
    using RBShapePtr = std::unique_ptr<RBShape>;
    using rb_shapes  = std::vector<RBShapePtr>;

    using RBPart  = rb::RigidBodyPart;
  ...
  inline size_t FixtureLevel1::noRigidBodies() const
  {
    return m_rigid_bodies.size();
  }

  inline FixtureLevel1::Forces FixtureLevel1::externalForces() const
  {
    return m_forces;
  }

  inline void FixtureLevel1::setGravity(Forces G) { m_forces = G; }

  inline FixtureLevel1::Point3
  FixtureLevel1::globalFramePosition(size_t rid) const
  {

    return m_rigid_bodies[rid]->globalFramePosition();
  }

  inline FixtureLevel1::Vector3 FixtureLevel1::globalVelocity(size_t rid) const
  {

    return m_rigid_bodies[rid]->velocity();
  }

  inline FixtureLevel1::RBMode FixtureLevel1::mode(size_t rid) const
  {

    return m_rigid_bodies[rid]->mode();
  }

  inline void FixtureLevel1::translateParent(size_t rid, Vector3 lin_trajectory)
  {
    m_rigid_bodies[rid]->m_object.translateParent(lin_trajectory);
  }

  inline void FixtureLevel1::addAcceleration(size_t rid, Vector3 accel)
  {
    m_rigid_bodies[rid]->addAcceleration(accel);
  }

  inline void FixtureLevel1::setVelocity(size_t rid, Vector3 velocity)
  {
    m_rigid_bodies[rid]->setVelocity(velocity);
  }

  inline size_t FixtureLevel1::createSphere(ValueType radius, Vector3 velocity,
                                            Vector3   translation,
                                            ValueType friction_coef)
  {
    ...
    // Create Shape
    m_rb_shapes.emplace_back(std::make_unique<rb::rb_shapes::Sphere>(radius));
  ...
  inline size_t FixtureLevel1::createFixedInfPlane(Vector3   normal,
                                                   Vector3   translation,
                                                   ValueType friction_coef)
  {
    // Create Rigid body
    ...
    // Create Shape
    m_rb_shapes.emplace_back(std::make_unique<rb::rb_shapes::InfPlane>(normal));
